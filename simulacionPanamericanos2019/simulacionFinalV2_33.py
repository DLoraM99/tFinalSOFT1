...
def MostrarPaises():
    lista=tkinter.Listbox(ventana,width=25,height=30)
    lista.insert(0)
    num=len(lista2)
    for i in range(num):
        a=(lista2[i])
...
def MostrarMarticipantes():
    lista=tkinter.Listbox(ventana, width = 5,height=30)
    lista.insert(0)
    num=len(lista3)
    for i in range(num):
        a=(lista3[i])
...
def MostrarMedallas():
    lista=tkinter.Listbox(ventana, width = 40, height = 30)
    lista.insert(0)
    num=len(lista5)
    for i in range(num):
        
        a=(repr(lista2[i]).rjust(20),repr(lista5[i]).rjust(5))
        lista.insert(tkinter.END,a)    
    lista.pack()
    lista.place(x=500,y=257)
  
    # LABELS
    M = tkinter.Label(text = "País   Oro   Plata   Bronce   Total", bg = "white")
    M.pack()
    M.place(x = 535, y = 235)
    
    o = tkinter.Label(text = " ", bg = "white")
    o.pack()
    o.place(x=450, y=258)
    
    p = tkinter.Label(text = " ", bg = "white")
    p.pack()
    p.place(x=450, y=273)
    
    b = tkinter.Label(text = " ", bg = "white")
    b.pack()
    b.place(x=450, y=288)
    
    t = tkinter.Label(text = " ", bg = "white")
    t.pack()
    t.place(x=450, y=303)
